...
@section('content')
    <div class="faq-category-index">
        <p><a class="btn btn-success" href="{{route("faq-category.create")}}"><i class="fa fa-plus-circle"></i> 创建分类</a></p>
        <div class="box">
                        ...
                        @if($faqCategories->count() > 0)
                            @foreach($faqCategories as $key=>$item)
                                <tr>
                                    ...
                                    <td class="operation">
                                        <a class="btn btn-sm btn-primary" href="{{ route('faq-category.edit',[$item->id]) }}" title="更新"><i class="fa fa-edit"></i> 更新</a>
                                        <form action="" method="POST" style="display: inline">
                                            {{ csrf_field() }}
                                            {{ method_field('DELETE') }}
                                            <a class="btn btn-sm btn-danger btn-delete" href="{{ route('faq-category.destroy',[$item->id]) }}" title="删除"><i class="fa fa-trash"></i> 删除</a>
                                        </form>
                                    </td>
                                    <td class="sort">
                                        @if($item->lft>1&&($item->lft!=$item->getParent()->lft+1))
                                            <a class="btn btn-sm btn-warning" href="{{ route('faq-category.move-up',[$item->id]) }}" title="上移"><i class="fa fa-arrow-up"></i> 上移</a>
                                        @else
                                        ...
                                        @if($item->id!=$item->getLastBrother()->id&&($item->rgt!=$item->getParent()->rgt-1))
                                            <a class="btn btn-sm btn-warning" href="{{ route('faq-category.move-down',[$item->id]) }}" title="下移"><i class="fa fa-arrow-down"></i> 下移</a>
                                        @else
